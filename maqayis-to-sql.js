/**
 * maqayis-to-sql.js  →  maqayis_import.sql
 * Yükle: wrangler d1 execute maqayis --file=maqayis_import.sql --remote
 */
const fs = require('fs');
...
const JSON_PATH = path.join(__dirname, 'data', 'maqayis.json');
const OUT_PATH  = path.join(__dirname, 'maqayis_import.sql');

if (!fs.existsSync(JSON_PATH)) {
  console.error('HATA: data/maqayis.json bulunamadı. Önce: node scripts/extract-maqayis.js');
  process.exit(1);
}
...
console.log(`✅ maqayis_import.sql (${sizeMB} MB, ${total} kayıt)`);
console.log('Yükle: wrangler d1 execute maqayis --file=maqayis_import.sql --remote');
